...
* Klasifikasi berbasis RGB murni (tanpa band inframerah/NIR) memiliki keterbatasan dalam membedakan kelas dengan karakteristik spektral tampak yang mirip, sehingga ambang batas (threshold) disusun secara empiris dan mungkin perlu penyesuaian untuk kawasan/citra lain.
* Nilai kecerahan dan rasio biru dari Danau Batur ini tumpang tindih dengan kelas lain (terutama vegetasi pekat di sekitarnya), sehingga rule umum untuk Badan Air sering gagal mengenali batasnya secara konsisten. Untuk mengatasi ini, dibutuhkan shapefile tambahan (danau\_batur.shp) yang di-rasterize dan digunakan untuk mengunci area Badan Air pada Danau Batur secara eksplisit, menggantikan pendekatan berbasis piksel murni. Konsekuensinya, akurasi kelas Badan Air pada laporan ini bergantung pada kualitas dan ketelitian batas shapefile tersebut, bukan murni hasil klasifikasi spektral.
* Kelas Lahan Terbuka/Semak Belukar (kode 3 pada skema awal) digabungkan ke dalam kelas Lahan Terbangun/Permukiman (kode 4) karena keduanya sulit dibedakan secara konsisten hanya dari kanal RGB (kecerahan tinggi dan selisih R–G kecil muncul pada tanah terbuka maupun material bangunan). Akibatnya, skema final hanya memuat 4 kelas tutupan lahan (Hutan/Vegetasi Lebat, Pertanian/Sawah, Lahan Terbangun/Permukiman, Badan Air/Perairan), dan kelas Lahan Terbangun/Permukiman menjadi lebih heterogen secara spektral karena mencakup dua tipe permukaan yang berbeda secara fisik (tanah/semak vs. material buatan).
* Titik sampel pelatihan dibangun dari hasil klasifikasi sementara (skema 5 kelas, sebelum penggabungan dan sebelum penyempurnaan dengan shapefile danau), sehingga sebagian titik pada area yang sebelumnya berkode Lahan Terbuka kini berada di kelas Lahan Terbangun pada peta final — konsistensi labelnya perlu dicatat sebagai potensi sumber bias.
* Hasil uji akurasi menunjukkan performa antar kelas tidak merata: Producer's Accuracy kelas Pertanian/Sawah hanya 19,18%, jauh di bawah kelas lain (Hutan 92,29%, Lahan Terbangun 78,44%, Badan Air 93,75%), yang mengindikasikan model rule-based RGB sering salah mengklasifikasikan piksel sawah/kebun sebagai kelas lain (kemungkinan besar tertukar dengan Hutan atau Lahan Terbangun akibat tumpang tindih rentang kecerahan). Overall Accuracy 80,04% dan Cohen's Kappa 0,6234 tergolong "cukup baik" (substantial agreement), namun angka ini banyak ditopang oleh kelas Badan Air dan Hutan yang lebih mudah dipisahkan secara spektral.
* Validasi model dilakukan terhadap data 2020–2023 (bukan data aktual 2026), sehingga metrik akurasi mencerminkan performa model pada tugas transisi historis, sebagai indikator kepercayaan terhadap proyeksi ke depan — bukan jaminan akurasi yang sama akan tercapai pada kondisi 2026 yang sebenarnya.
